...
def version_info():
    # What is actually running here, not whatever tag happened to be the
    # newest release (fork #1108) — `--version` reporting the latter made the
    # CLI claim a version the install did not have.
...
class CliParameter(object):
    ...
    def arg_parser(self):
        ...
        parser.add_argument('-o', metavar='path', help='path and name Calibre-Web NextGen logfile')
        parser.add_argument('-v', '--version', action='version', help='Shows version number '
                                                                      'and exits Calibre-Web NextGen',
        ...
        parser.add_argument('-s', metavar='user:pass',
                            help='Sets specific username to new password and exits Calibre-Web NextGen')
        ...
        parser.add_argument('-d', action='store_true', help='Dry run of updater to check file permissions '
                                                            'in advance and exits Calibre-Web NextGen')
