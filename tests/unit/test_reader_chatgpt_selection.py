...
def test_selection_toolbar_uses_the_font_awesome_openai_icon():
    assert "FontAwesomeIcon" in READER
    assert "faOpenai" in READER
    toolbar = READER.split('className={styles.selectionBar}', 1)[1].split('</div>', 1)[0]
    assert "openSelectedTextInChatGpt" in toolbar
