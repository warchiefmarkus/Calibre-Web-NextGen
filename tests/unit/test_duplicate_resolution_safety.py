...
class TestD10HiddenBooksExcludedFromScan:
    def test_get_common_filters_excludes_user_hidden_books(self):
        # D10 originally pinned a second hand-written policy here. That copy
        # later drifted on My Library membership, so explicit-user callers must
        # now delegate to the canonical filter that owns UserHiddenBook too.
        adapter = _func_src("get_common_filters")
        db_src = (REPO_ROOT / "cps" / "db.py").read_text()
        assert "return calibre_db.common_filters(" in adapter
        assert "UserHiddenBook" in db_src and "hidden_filter" in db_src
        assert "ub.ArchivedBook" not in adapter
        assert "UserHiddenBook" not in adapter
    ...
    def test_get_common_filters_has_allow_show_hidden_parity(self):
        ...
        assert "allow_show_hidden=allow_show_hidden" in src, (
            "the adapter must forward allow_show_hidden to "
